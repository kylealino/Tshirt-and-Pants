...
<?=form_open('me-fg-packing-appr','class="needs-validation-search" id="myfrmsearchrec_appr" ');?>
						...
						<tbody>
							...
							if($rlist !== ''):
								...
								<tr bgcolor="<?=$bgcolor;?>" <?=$on_mouse;?>>
									...
									<td>
										<button class="btn btn-dgreen btn-sm" onclick="javascript:save_for_approve_fgpack('<?=$mtkn_fgpacktr;?>');" <?=$dis;?>> <i class="bi bi-print"></i> For Approved</button>
							      	</td>
...
<script type="text/javascript"> 
	...
	function __myredirected_rsearch_appr(mobj) { 
		try { 
			...
			type: "POST",
			url: '<?=site_url();?>me-fg-packing-appr',
	...
	function save_for_approve_fgpack(mtkn_fgpacktr){
		try { 
            __mysys_apps.mepreloader('mepreloaderme',true);
                  ...
                  jQuery.ajax({ // default declaration of ajax parameters
                    type: "POST",
                    url: '<?=site_url();?>me-fg-packing-appr-save',
                    context: document.body,
                    data: eval(mparam),
                    global: false,
                    cache: false,
