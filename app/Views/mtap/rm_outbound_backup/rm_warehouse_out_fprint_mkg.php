...
$pdf = new Mypdf();
$pdf->AliasNbPages();
$pdf->SetTitle('POST SHIPPING DOCUMENT - MKG '.$crpl_code);
$pdf->AddPage();
...
$pdf->SetXY(5,18);
$pdf->SetFont('Dot','',11);
$pdf->Cell(206,5,'RM OUTBOUND',0,0,'C');
...
$pdf->Cell(68,4,'DESCRIPTION',1,0,'C');
$pdf->SetFont('Dot','',7);
...
$pdf->SetY(-12);
$pdf->SetFont('Dot','',10);
$pdf->Cell(0,10,'Page '.$pdf->PageNo().'/{nb}'.' of SHIPPING DOC - MKG: '.$crpl_code,0,0,'C');
...
foreach($q3->getResultArray() as $row){
	...
	if($ART_HIERC1 == 'STORE USE'){
		...
		if($ART_SKU == 'KG'){
 			...
 			$hsy_bundle += $total_pcs;
 		}
 		else if($ART_SKU == 'PLASTIC'){
 			$hsy_plastic += $total_pcs;
 		}
 		else if($ART_SKU == 'PCS'){
 			$hsy_pcs += $total_pcs;
 		}
		...
		if($ART_SKU == 'KG'){
 			...
 			$lsg_bundle += $total_pcs;
 		}
 		else if($ART_SKU == 'PLASTIC'){
 			$lsg_plastic += $total_pcs;
 		}
 		else if($ART_SKU == 'PCS'){
 			$lsg_pcs += $total_pcs;
 		}
		...
		if($ART_SKU == 'KG'){
 			...
 			$hqe_bundle += $total_pcs;
 		}
 		else if($ART_SKU == 'PLASTIC'){
 			$hqe_plastic += $total_pcs;
 		}
 		else if($ART_SKU == 'PCS'){
 			$hqe_pcs += $total_pcs;
 		}
		...
		if($ART_SKU == 'KG'){
 			...
 			$aycc_bundle += $total_pcs;
 		}
 		else if($ART_SKU == 'PLASTIC'){
 			$aycc_plastic += $total_pcs;
 		}
 		else if($ART_SKU == 'PCS'){
 			$aycc_pcs += $total_pcs;
 		}
	...
	if($ART_SKU == 'KG'){
		...
		$total_sack += $total_pcs;
	}
	else if($ART_SKU == 'ROLL'){
		$total_roll += $total_pcs;
	}
	else if($ART_SKU == 'BUNDLE'){
		$total_bundle += $total_pcs;
	}
	else if($ART_SKU == 'PLASTIC'){
		$total_plastic += $total_pcs;
	}
	else if($ART_SKU == 'PCS'){
		$total_pcs_breakdown += $total_pcs;
	}
	else if($ART_SKU == 'CTNS'){
		$total_ctns += $total_pcs;
	}
 	...
 	if($po_class == 1){
 		if($ART_SKU == 'KG'){
 			...
 			$imp_bundle += $total_pcs;
 		}
 		else if($ART_SKU == 'PLASTIC'){
 			$imp_plastic += $total_pcs;
 		}
 		else if($ART_SKU == 'PCS'){
 			$imp_pcs += $total_pcs;
 		}
 		...
 		if($ART_SKU == 'KG'){
 			...
 			$loc_bundle += $total_pcs;
 		}
 		else if($ART_SKU == 'PLASTIC'){
 			$loc_plastic += $total_pcs;
 		}
 		else if($ART_SKU == 'PCS'){
 			$loc_pcs += $total_pcs;
 		}
		...
		foreach($q->getResultArray() as $rr){
			...
			$str = "
				SELECT
					...
					b.`ART_UOM`
				FROM
					{$this->db_erp}.`rm_inv_rcv` a
				LEFT JOIN
					{$this->db_erp}.`mst_article` b
				ON
					a.`mat_rid` = b.`recid`
				LEFT JOIN
					{$this->db_erp}.`rm_inv_rcv_item` c
				ON
					a.`recid` = c.`wshe_inv_id`
				WHERE
					a.`wob_barcde` = '{$wob_barcde}'
				AND
			...
			if($q->getNumRows() > 0){
				...
				foreach($q->getResultArray() as $zz){
					...
					$data = $zz['ART_CODE'].'x|x'.$zz['ART_DESC'].'x|x'.$zz['ART_BARCODE1'].'x|x'.$qty.'x|x'.$zz['ART_UPRICE'].'x|x'.$crpl_id.'x|x'.number_format($qty * $convf,2).'x|x'.$zz['ART_UOM'].'x|x'.$box_no.'x|x'.$count_item.'x|x'.$item_no.'x|x'.number_format($zz['qty'],2);
...
for($i = 0; $i < count($data_rows); $i++){
	...
	if($Y < 226){
		...
		if($box_no != $xboxno){
			...
			$pdf->Cell(12,5,number_format($total_pcs,2),1,0,'C');
			$pdf->Cell(10,5,$ART_UOM,1,0,'C');
			$pdf->Cell(26,5,$ART_BARCODE1,1,0,'C');
			$pdf->Cell(27,5,$ART_CODE,1,0,'L');
			$pdf->SetFont('Dot','',8);
			$pdf->Cell(68,5,$ART_DESC,1,0,'L');
			$pdf->SetFont('Dot','',10);
			// $pdf->Cell(12,5,$qty,$border,0,'C'); 
			// $pdf->Cell(12,5,$conf,1,0,'C');
...
if($Y < 186){
	...
	$pdf->SetFont('Dot','',10);
	$pdf->SetXY(16,$Y);
	$pdf->Cell(18,5,number_format($total_qty,2),'B',0,'L');
