		          	...
		          	<table class="table table-bordered table-hover table-sm text-center" id="tbl-out-recs">
			            ...
			            <tbody id="tbody-inv-items-recs">
			              	<?php 
								...
								$header = $row['header'];
					           	$print = 'fgp-out-print';
					           	$fprint = 'fgp-out-fprint';

			              	?>
			              	<tr bgcolor="<?= $bgcolor ?>" <?=$on_mouse?> >
			              		<td>
			              		<?=anchor('fgp-out/?mtkn_trxno=' . $txt_mtknr, '<i class="bi bi-pencil-square"></i>',' class="btn bg-dgreen btn-sm" ');?>	
			              		</td>
...
$('.btn-whout-done').on('click',function(){
	try{
		...
		let url = (btn_type == 'D')?'<?=site_url()?>fgp-out-done':'<?=site_url()?>fgp-out-revert';
