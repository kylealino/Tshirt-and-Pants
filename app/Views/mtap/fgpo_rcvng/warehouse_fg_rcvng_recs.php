						...
						<tbody>
							...
							if($rlist !== ''):
								...
								foreach($rlist as $row): 
									...
									$txt_mtknr = hash('sha384', $row['po_sysctrlno'] . $mpw_tkn);
									//$dis_edt = ($row['post_tag'] == 'Y' ? "isDisabled" : '');
									$variance = '<span class="badge bg-dgreen"> NO DATA </span>';
									$dt_rcv_qty =  $row['dt_rcv_qty'];
									$hd_rcv_qty = $row['hd_rcv_qty'];
									$po_sysctrlno = ($row['po_sysctrlno']);
									if($dt_rcv_qty == 0){
									  $variance = $variance;
									}
									elseif($dt_rcv_qty > $hd_rcv_qty || $dt_rcv_qty < $hd_rcv_qty){
									  $variance = '<span class="badge bg-danger"> VARIANCE OCCUR </span>';
									}
									elseif($dt_rcv_qty == $hd_rcv_qty ){
								...
								<tr>
									...
									<td nowrap="nowrap">
										<?php if($dt_rcv_qty != $hd_rcv_qty ):?>
											
									<?=anchor('fgpo-receiving/?po_sysctrlno=' . $po_sysctrlno, 'PROCESS',' class="btn btn-dgreen p-1 pb-0 mebtnpt1 btn-sm"');?>
									...
									<td nowrap="nowrap">
										<button title="View items" class="btn btn-dgreen-ol btn-xs gr-btn-view-items"  data-grno= "<?=$row['po_sysctrlno'];?>" value="<?=$txt_mtknr?>" type="button" ><i class="bi bi-eye-fill"></i> View</button>
									</td>
								</tr>
								<?php 
								$nn++;
