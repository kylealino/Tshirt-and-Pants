...
<?=form_open('me-purchase-recs','class="needs-validation-search" id="myfrmsearchrec" ');?>
							...
							if($rlist !== ''):
								...
								<tr bgcolor="<?=$bgcolor;?>" <?=$on_mouse;?>>
									<td class="text-center" nowrap>
										<?=anchor('me-purchase-vw/?mtkn_trxno=' . $mtkn_potr, '<i class="bi bi bi-pencil"></i>',' class="btn btn-dgreen p-1 pb-0 mebtnpt1 btn-sm"');?>
                                    ...
                                    echo("<button onclick=\"window.open('me-purchase-print?mtkn_potr=$mtkn_potr')\" class=\"btn btn-primary btn-sm\"><i class=\"fa fa-print\"></i> Print list</button>");
										//echo("<button onclick=\"javascript:__mbtn_po_print('','');\" class=\"btn btn-primary btn-sm\"><i class=\"fa fa-print\"></i> Print list</button>");
										//echo("<button onclick=\"window.open('me-purchase-print-temp')\" class=\"btn btn-primary btn-sm\"><i class=\"fa fa-print\"></i> Print list</button>");
									...
									<button  title="Hint:Barcode Generation will done once." class="btn btn-danger btn-xs" onclick="javascript:po_bcode_gen('<?=$mtkn_potr;?>');" <?=$dis;?>> <i class="bi bi-print"></i> Generate</button>

							      	</td>
							      	<td>
									
									<button class="btn btn-success btn-xs" onclick="javascript:__mbtn_po_bdownload('<?=$row['po_sysctrlno'];?>','<?=$mtkn_wshe;?>');" <?=$dis2;?>> <i class="bi bi-print"></i> Download</button>
	...
	function po_bcode_gen(mtkn_potr){
		try { 
            __mysys_apps.mepreloader('mepreloaderme',true);
                  ...
                  jQuery.ajax({ // default declaration of ajax parameters
                    type: "POST",
                    url: '<?=site_url();?>me-purchase-bar-generate',
                    context: document.body,
                    data: eval(mparam),
                    global: false,
                    cache: false,
	...
	function __mbtn_po_bdownload(po_sysctrlno,mtkn_wshe){
		try { 
            __mysys_apps.mepreloader('mepreloaderme',true);
                  ...
                  jQuery.ajax({ // default declaration of ajax parameters
                    type: "POST",
                    url: '<?=site_url();?>me-purchase-barcode-dl',
                    context: document.body,
                    data: eval(mparam),
                    global: false,
                    cache: false,
	...
	function __mbtn_po_print(po_sysctrlno,mtkn_wshe){
		try { 
            __mysys_apps.mepreloader('mepreloaderme',true);
                  ...
                  jQuery.ajax({ // default declaration of ajax parameters
                    type: "POST",
                    url: '<?=site_url();?>me-purchase-print-temp',
                    context: document.body,
                    data: eval(mparam),
                    global: false,
                    cache: false,
