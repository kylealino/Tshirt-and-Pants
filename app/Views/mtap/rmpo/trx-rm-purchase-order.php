...
$str = "
    ...
    from {$this->db_erp}.`gw_rm_po_hd` aa 
...
<main id="main">
    ...
    <div class="pagetitle">
        <h1>RM Purchase Order</h1>
        <nav>
            <ol class="breadcrumb">
                <li class="breadcrumb-item"><a href="index.html">Home</a></li>
                <li class="breadcrumb-item active">RM Purchase Order</li>
            </ol>
            </nav>
    </div><!-- End Page Title -->
...
<script type="text/javascript">
    ...
    $('#mbtn_mn_NTRX').click(function() { 
        ...
        if (userselection == true){
            window.location = '<?=site_url();?>me-rm-purchase-vw';
         }
        else{
            $.hideLoading();
            return false;
  ...
  $("#mbtn_mn_Save").click(function(e){
    ...
    //__mysys_apps.mepreloader('mepreloaderme',true);
      ...
      $.ajax({ 
        type: "POST",
        url: '<?=site_url();?>me-rm-purchase-save',
        context: document.body,
        data: eval(mparam),
        global: false,
        cache: false,
...
function mypo_view_recs(mtkn_whse){ 
    ...
    ajaxRequest = jQuery.ajax({
        url: "<?=site_url();?>me-rm-purchase-view",
        type: "post",
        data: {
            mtkn_whse: mtkn_whse
        }
