...
<?=form_open('me-rm-purchase-appr','class="needs-validation-search" id="myfrmsearchrec_appr" ');?>
						...
						<tbody>
							...
							if($rlist !== ''):
								...
								<tr bgcolor="<?=$bgcolor;?>" <?=$on_mouse;?>>
									...
									<td>
										<button class="btn btn-dgreen btn-sm" onclick="javascript:save_for_approve('<?=$mtkn_potr;?>');" <?=$dis;?>> <i class="bi bi-print"></i> For Approval</button>
							      	</td>
...
<script type="text/javascript"> 
	...
	function __myredirected_rsearch_appr(mobj) { 
		try { 
			...
			type: "POST",
			url: '<?=site_url();?>me-rm-purchase-appr',
	...
	function save_for_approve(mtkn_potr){
		try { 
            __mysys_apps.mepreloader('mepreloaderme',true);
                  ...
                  jQuery.ajax({ // default declaration of ajax parameters
                    type: "POST",
                    url: '<?=site_url();?>me-rm-purchase-appr-save',
                    context: document.body,
                    data: eval(mparam),
                    global: false,
                    cache: false,
