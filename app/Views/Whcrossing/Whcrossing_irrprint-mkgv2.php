...
$str = "
			...
			sum(dt.`qty`) expected,
			dt.`convf`,
			IFNULL((SELECT SUM(qty) FROM  trx_po_dt WHERE  po_wshe_id = 154 AND `art_rid` = dt.`art_rid` AND  po_sysctrlno IN ($trxpos) ),0) w11ov,
...
foreach($q3->getResultArray() as $row){
	...
	for($i = 0; $i < count($item); $i++){
		...
		if($Y < 182){
			...
			$pdf->Cell(20,8,'VARIANCE',1,0,'C','true');
			// $pdf->Cell(20,4,'DUMPING AREA','T,L,R',0,'C','true');
			$pdf->Cell(25,4,'W1-G1',1,0,'C','true');
			$pdf->Cell(25,4,'W1-G2',1,0,'C','true');
			$pdf->Cell(25,4,'W2-G3',1,0,'C','true');
			$pdf->Cell(25,4,'W2-G4',1,0,'C','true');
			$pdf->Cell(25,4,'W11-G5',1,0,'C','true');
			$pdf->Cell(25,4,'W11-G6',1,0,'C','true');
			$pdf->SetXY(65.5,$Y+8); 
			$pdf->Cell(11,4,'RECEIVE','B,R,L',0,'C','true'); 
			$pdf->SetXY(122.5,$Y+8); 
			// $pdf->Cell(20,4,'(WH11B)','B,L,R',0,'C','true');
			$pdf->Cell(12,4,'XPCTD',1,0,'C','true');
			$pdf->Cell(12,4,'ACTUAL',1,0,'C','true');
