...
$str = "
			...
			sum(dt.`qty`) expected,
			dt.`convf`,
			IFNULL((SELECT SUM(qty) FROM  trx_po_dt WHERE  po_wshe_id = 165 AND `art_rid` = dt.`art_rid` AND  po_sysctrlno IN ($trxpos) ),0) wtagum,
			IFNULL((SELECT SUM(qty) FROM  trx_po_dt WHERE  po_wshe_id = 168 AND `art_rid` = dt.`art_rid` AND  po_sysctrlno IN ($trxpos) ),0) wovflow,
			'SA' ART_SKU,
			po.`asstd_tag`,
			dt.`po_sysctrlno`
			...
			WHERE aa.`agpo_sysctrlno` = '$PPO_CTRLNO'  AND po.`plnt_id`= '6'
			GROUP BY dt.`art_rid`";
...
$pdf->Cell(24,4,'CD-TAGUM',1,0,'C','true');
	...
	for($i = 0; $i < count($item); $i++){
		...
		if($Y < 182){
			...
			if($_recid != $xrecid){
				...
				$pdf->Cell(12,5,number_format($wtagum,0),$border,0,'C'); //CD-TAGUM xpctd
				$pdf->Cell(12,5,'',$border,0,'L'); //CD-TAGUM actual
				...
				$pdf->Cell(10.5,5,'',$border,0,'L'); //wovflow actual
				$pdf->Cell(12,5,'',$border,0,'C'); //CD-TAGUM xpctd
				$pdf->Cell(12,5,'',$border,0,'L'); //CD-TAGUM actual
...
if($Y < 191){
	...
	$pdf->Cell(12,5,number_format($gtwtagum,0),1,0,'C'); //CD-TAGUM xpctd
	$pdf->Cell(12,5,'',1,0,'L'); //CD-TAGUM actual
	...
	$pdf->SetXY(5,$Y+10);
	$pdf->Cell(40,8,'CD-TAGUM','R,L',0,'C'); 
