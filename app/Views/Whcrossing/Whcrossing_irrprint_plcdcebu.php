...
$str = "
			...
			sum(dt.`qty`) expected,
			dt.`convf`,
			IFNULL((SELECT SUM(qty) FROM  trx_po_dt WHERE  po_wshe_id = 170 AND `art_rid` = dt.`art_rid` AND  po_sysctrlno IN ($trxpos) ),0) wcdcebu,
			IFNULL((SELECT SUM(qty) FROM  trx_po_dt WHERE  po_wshe_id = 167 AND `art_rid` = dt.`art_rid` AND  po_sysctrlno IN ($trxpos) ),0) wovflow,
			'SA' ART_SKU,
			po.`asstd_tag`,
			dt.`po_sysctrlno`
			...
			WHERE aa.`agpo_sysctrlno` = '$PPO_CTRLNO'  AND po.`plnt_id`= '3'
			GROUP BY dt.`art_rid`";
...
$pdf->Cell(21,4,'OVERFLOW',1,0,'C','true');
$pdf->Cell(24,4,'CD-CEBU',1,0,'C','true');
	...
	for($i = 0; $i < count($item); $i++){
		...
		if($Y < 182){
			...
			if($_recid != $xrecid){
				...
				$pdf->Cell(10.5,5,number_format($wovflow,0),$border,0,'C'); //wovflow xpctd
				$pdf->Cell(10.5,5,'',$border,0,'L'); //wovflow actual
				$pdf->Cell(12,5,number_format($wcdcebu,0),$border,0,'C'); //CD-CEBU xpctd
				$pdf->Cell(12,5,'',$border,0,'L'); //CD-CEBU actual
				...
				$pdf->Cell(10.5,5,'','C'); //wovflow xpctd
				$pdf->Cell(10.5,5,'',$border,0,'L'); //wovflow actual
				$pdf->Cell(12,5,'',$border,0,'C'); //CD-CEBU xpctd
				$pdf->Cell(12,5,'',$border,0,'L'); //CD-CEBU actual
...
if($Y < 191){
	...
	$pdf->Cell(10.5,5,number_format($gtwovflow,0),1,0,'C'); //wovflow xpctd
	$pdf->Cell(10.5,5,'',1,0,'L'); //wovflow actual
	$pdf->Cell(12,5,number_format($gtwcdcebu,0),1,0,'C'); //CD-CEBU xpctd
	$pdf->Cell(12,5,'',1,0,'L'); //CD-CEBU actual
	...
	$pdf->Cell(12.5,5,number_format($gtwovflow,0),1,0,'C'); //wovflow xpctd
	$pdf->Cell(12.5,5,'',1,0,'L'); //wovflow actual
	$pdf->Cell(12,5,number_format($gtwcdcebu,0),1,0,'C'); //CD-CEBU xpctd
	$pdf->Cell(12,5,'',1,0,'L'); //CD-CEBU actual
	...
	$pdf->SetXY(5,$Y+5);
	$pdf->Cell(147.5,5,'BY SIGNING THIS DOCUMENTS IT INDICATES THAT THE ENDORSEMENT OF ITEMS BY THE GROUP.',1,1,'C'); 
	$pdf->SetXY(5,$Y+10);
	$pdf->Cell(40,8,'CD-CEBU','R,L',0,'C'); 
	$pdf->Cell(45,8,'--','R,L',0,'C'); 
	$pdf->Cell(45,8,'--','R,L',0,'C'); 
