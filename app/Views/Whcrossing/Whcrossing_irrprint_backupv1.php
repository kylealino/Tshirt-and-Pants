...
$str = "
			...
			sum(dt.`qty`) expected,
			dt.`convf`,
			IFNULL((SELECT SUM(qty) FROM  trx_po_dt WHERE  po_wshe_id = 163 AND `art_rid` = dt.`art_rid` AND  po_sysctrlno IN ($trxpos) ),0) w11ov,
			IFNULL((SELECT SUM(qty) FROM  trx_po_dt WHERE  po_wshe_id = 157 AND `art_rid` = dt.`art_rid` AND  po_sysctrlno IN ($trxpos) ),0) w1g1,
			IFNULL((SELECT SUM(qty) FROM  trx_po_dt WHERE  po_wshe_id = 158 AND `art_rid` = dt.`art_rid` AND  po_sysctrlno IN ($trxpos) ),0) w1g2,
			IFNULL((SELECT SUM(qty) FROM  trx_po_dt WHERE  po_wshe_id = 159 AND `art_rid` = dt.`art_rid` AND  po_sysctrlno IN ($trxpos) ),0) w2g3,
			IFNULL((SELECT SUM(qty) FROM  trx_po_dt WHERE  po_wshe_id = 160 AND `art_rid` = dt.`art_rid` AND  po_sysctrlno IN ($trxpos) ),0) w2g4,
			IFNULL((SELECT SUM(qty) FROM  trx_po_dt WHERE  po_wshe_id = 161 AND `art_rid` = dt.`art_rid` AND  po_sysctrlno IN ($trxpos) ),0) w11g5,
			IFNULL((SELECT SUM(qty) FROM  trx_po_dt WHERE  po_wshe_id = 162 AND `art_rid` = dt.`art_rid` AND  po_sysctrlno IN ($trxpos) ),0) w11g6,
			'SA' ART_SKU,
			po.`asstd_tag`,
			dt.`po_sysctrlno`
...
$pdf->SetXY(5,50); 
$pdf->Cell(23,12,'ITEM CODE',1,0,'C','true'); 
$pdf->Cell(23,12,'BOX CONTENT',1,0,'C','true'); 
$pdf->Cell(10,6,'TOTAL','T,R,L',0,'C','true'); 
$pdf->Cell(10.5,6,'PCS PER',1,0,'C','true'); 
$pdf->Cell(51,4,'INSPECTION RESULT',1,0,'C','true');
$pdf->Cell(150,4,'QUANTITY ENDORSEMENT',1,0,'C','true');  
$pdf->SetXY(51,56);
$pdf->Cell(10,6,'BOXES','B,L,R',0,'C','true'); 
$pdf->Cell(10.5,6,'BOX','B,L,R',0,'C','true'); 
$pdf->SetXY(71.5,54);
$pdf->Cell(17,4,'GOOD','T,R,L',0,'C','true'); 
$pdf->Cell(17,8,'DAMAGE',1,0,'C','true'); 
$pdf->Cell(17,8,'VARIANCE',1,0,'C','true');
// $pdf->Cell(20,4,'DUMPING AREA','T,L,R',0,'C','true');
$pdf->Cell(24,4,'W11-OVERFLOW',1,0,'C','true');
$pdf->Cell(21,4,'W1-G1',1,0,'C','true');
$pdf->Cell(21,4,'W1-G2',1,0,'C','true');
$pdf->Cell(21,4,'W2-G3',1,0,'C','true');
$pdf->Cell(21,4,'W2-G4',1,0,'C','true');
$pdf->Cell(21,4,'W11-G5',1,0,'C','true');
$pdf->Cell(21,4,'W11-G6',1,0,'C','true');
...
foreach($q3->getResultArray() as $row){
	...
	for($i = 0; $i < count($item); $i++){
		...
		if($Y < 182){
			...
			$pdf->SetXY(5,$Y); 
			if($_recid != $xrecid){
				$pdf->Cell(23,$yaxis,$_ART_CODE,$border,0,'L'); 
				$pdf->Cell(23,$yaxis,$_ART_DESC,1,0,'L'); 
				$pdf->Cell(10,5,number_format($tboxes,0),$border,0,'C'); 
				$pdf->Cell(10.5,5,$convf,1,0,'L'); 
				$pdf->Cell(17,5,'',1,0,'L'); //good receive
				$pdf->Cell(17,5,'',1,0,'L');  // damge
				$pdf->Cell(17,5,'',1,0,'L');  // variance
				// $pdf->Cell(20,5,'',1,0,'L');  // dump area
				$pdf->Cell(12,5,number_format($w11ov,0),$border,0,'C'); //w11-overflow xpctd
				$pdf->Cell(12,5,'',$border,0,'L'); //w11-overflow actual
				$pdf->Cell(10.5,5,number_format($w1g1,0),$border,0,'C'); //w1g1 xpctd
				$pdf->Cell(10.5,5,'',$border,0,'L'); //w1g1 actual
				$pdf->Cell(10.5,5,number_format($w1g2,0),$border,0,'C'); //w1g2 xpctd
				$pdf->Cell(10.5,5,'',$border,0,'C'); //w1g2 actual
				$pdf->Cell(10.5,5,number_format($w2g3,0),$border,0,'C');//w2g3 xpctd
				$pdf->Cell(10.5,5,'',$border,0,'L'); //w2g3 actual
				$pdf->Cell(10.5,5,number_format($w2g4,0),$border,0,'C');//w2g4 xpctd
				$pdf->Cell(10.5,5,'',$border,0,'C'); //w2g4 actual
				$pdf->Cell(10.5,5,number_format($w11g5,0),$border,0,'C');//w11g5 xpctd
				$pdf->Cell(10.5,5,'',$border,0,'C'); //w11g5actual
				$pdf->Cell(10.5,5,number_format($w11g6,0),$border,0,'C'); //w11g6 xpctd
				$pdf->Cell(10.5,5,'',$border,0,'C'); //w11g6 actual
				...
				$pdf->Cell(17,5,'',1,0,'L');  // variance
				// $pdf->Cell(20,5,'',1,0,'L');  // dump area
				$pdf->Cell(12,5,'',$border,0,'C'); //w11-overflow xpctd
				$pdf->Cell(12,5,'',$border,0,'L'); //w11-overflow actual
				$pdf->Cell(10.5,5,'','C'); //w1g1 xpctd
				$pdf->Cell(10.5,5,'',$border,0,'L'); //w1g1 actual
				$pdf->Cell(10.5,5,'',$border,0,'C'); //w1g2 xpctd
				$pdf->Cell(10.5,5,'',$border,0,'C'); //w1g2 actual
				$pdf->Cell(10.5,5,'',$border,0,'C');//w2g3 xpctd
				$pdf->Cell(10.5,5,'',$border,0,'L'); //w2g3 actual
				$pdf->Cell(10.5,5,'',$border,0,'C');//w2g4 xpctd
				$pdf->Cell(10.5,5,'',$border,0,'C'); //w2g4 actual
				$pdf->Cell(10.5,5,'',$border,0,'C');//w11g5 xpctd
				$pdf->Cell(10.5,5,'',$border,0,'C'); //w11g5actual
				$pdf->Cell(10.5,5,'',$border,0,'C'); //w11g6 xpctd
				$pdf->Cell(10.5,5,'',$border,0,'C'); //w11g6 actual
			...
			$pdf->Cell(23,12,'BOX CONTENT',1,0,'C','true'); 
			$pdf->Cell(10,6,'TOTAL','T,R,L',0,'C','true'); 
			$pdf->Cell(10.5,6,'PCS PER','T,R,L',0,'C','true'); 
			$pdf->Cell(51,4,'INSPECTION RESULT',1,0,'C','true');
			$pdf->Cell(150,4,'QUANTITY ENDORSEMENT',1,0,'C','true');  
			$pdf->SetXY(51,$Y+6);
			$pdf->Cell(10,6,'BOXES','B,L,R',0,'C','true'); 
