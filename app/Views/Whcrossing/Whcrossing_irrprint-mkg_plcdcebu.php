...
$str = "
			...
			IFNULL((SELECT SUM(qty) FROM  trx_po_dt WHERE  po_wshe_id = 170 AND `art_rid` = dt.`art_rid` AND  po_sysctrlno IN ($trxpos) ),0) w1g1,
			'SA' ART_SKU,
			po.`asstd_tag`,
			dt.`po_sysctrlno`
...
$pdf->Cell(21,4,'CD-CEBU',1,0,'C','true');
	...
	if ($asstd_tag == 'Y') {
		...
		$str = "
			SELECT 
				...
				IFNULL((SELECT SUM(qty) FROM  trx_po_dt WHERE  po_wshe_id = 170 AND `art_rid` = c.`art_rid` AND `convf` = a.`qty` AND  po_sysctrlno IN ($trxpos) ),0) w1g1
