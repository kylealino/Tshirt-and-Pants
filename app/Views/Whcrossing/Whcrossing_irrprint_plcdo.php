...
$str = "
			...
			sum(dt.`qty`) expected,
			dt.`convf`,
			IFNULL((SELECT SUM(qty) FROM  trx_po_dt WHERE  po_wshe_id = 166 AND `art_rid` = dt.`art_rid` AND  po_sysctrlno IN ($trxpos) ),0) wcdo,
			...
			WHERE aa.`agpo_sysctrlno` = '$PPO_CTRLNO'  AND po.`plnt_id`= '4'
			GROUP BY dt.`art_rid`";
...
$pdf->Cell(24,4,'CD-CD0',1,0,'C','true');
	...
	for($i = 0; $i < count($item); $i++){
		...
		if($Y < 182){
			...
			if($_recid != $xrecid){
				...
				$pdf->Cell(12,5,number_format($wcdo,0),$border,0,'C'); //CD-CDO xpctd
				$pdf->Cell(12,5,'',$border,0,'L'); //CD-CDO actual
				...
				$pdf->Cell(10.5,5,'',$border,0,'L'); //wovflow actual
				$pdf->Cell(12,5,'',$border,0,'C'); //CD-CDO xpctd
				$pdf->Cell(12,5,'',$border,0,'L'); //CD-CDO actual
...
if($Y < 191){
	...
	$pdf->Cell(12,5,number_format($gtwcdo,0),1,0,'C'); //CD-CDO xpctd
	$pdf->Cell(12,5,'',1,0,'L'); //CD-CDO actual
