	...
	if ($asstd_tag == 'Y') {
		...
		$str = "
			SELECT 
				...
				SUM(c.`qty`) tot_box,
				SUM(c.`convf`) convf,
				IFNULL((SELECT SUM(qty) FROM  trx_po_dt WHERE  po_wshe_id = 163 AND `art_rid` = c.`art_rid` AND `convf` = a.`qty` AND  po_sysctrlno IN ($trxpos) ),0) w11ov,
				IFNULL((SELECT SUM(qty) FROM  trx_po_dt WHERE  po_wshe_id = 157 AND `art_rid` = c.`art_rid` AND `convf` = a.`qty` AND  po_sysctrlno IN ($trxpos) ),0) w1g1,
				IFNULL((SELECT SUM(qty) FROM  trx_po_dt WHERE  po_wshe_id = 158 AND `art_rid` = c.`art_rid` AND `convf` = a.`qty` AND  po_sysctrlno IN ($trxpos) ),0) w1g2,
				IFNULL((SELECT SUM(qty) FROM  trx_po_dt WHERE  po_wshe_id = 159 AND `art_rid` = c.`art_rid` AND `convf` = a.`qty` AND  po_sysctrlno IN ($trxpos) ),0) w2g3,
				IFNULL((SELECT SUM(qty) FROM  trx_po_dt WHERE  po_wshe_id = 160 AND `art_rid` = c.`art_rid` AND `convf` = a.`qty` AND  po_sysctrlno IN ($trxpos) ),0) w2g4,
				IFNULL((SELECT SUM(qty) FROM  trx_po_dt WHERE  po_wshe_id = 161 AND `art_rid` = c.`art_rid` AND `convf` = a.`qty` AND  po_sysctrlno IN ($trxpos) ),0) w11g5,
				IFNULL((SELECT SUM(qty) FROM  trx_po_dt WHERE  po_wshe_id = 162 AND `art_rid` = c.`art_rid` AND `convf` = a.`qty` AND  po_sysctrlno IN ($trxpos) ),0) w11g6
	...
	for($i = 0; $i < count($item); $i++){
		...
		if($Y < 182){
			...
			if($_recid != $xrecid){
				...
				$pdf->Cell(12,5,number_format($w11ov,0),1,0,'C'); //w11-overflow xpctd
