...
$gitype  = $mydataz->lk_Active_GI_type();
$fldType = "";
?>
  ...
  <div class="pagetitle">
      ...
      <nav>
          <ol class="breadcrumb">
              ...
              <li class="breadcrumb-item active">GI Entry</li>
          </ol>
          </nav>
  </div><!-- End Page Title -->
          ...
          <div class="container-fluid">
              <div class="row" >
                    ...
                    <div class="card-body">
                      ...
                      <div class="mt-2 col-12">
                        <label for="gi-type">Type</label>
                         <?=$mylibzsys->mypopulist_2($gitype,$fldType,'gi-type','class="form-control form-control-sm" name="gi-type"','','');?>
                      </div>
                       <div class="mt-2 col-12">
                        <label for="gi-remarks">Remarks</label>
                          <textarea name="gi-remarks"  class="form-control form-control-sm" id="gi-remarks" cols="30" rows="5"></textarea>
                      </div>
                     <div class="input-group input-group-sm  mt-3">
                       ...
                       <div class="input-group-prepend" id="basic-addon1">
                         <button type="button" id="btn-upload-wshe-gi" class="btn btn-dgreen btn-sm m-0 rounded-0 rounded-end" ><i class="bi bi-upload"></i> Upload</button>
                       </div>
                     </div>
                    <div class="form-text mb-3">3333 for out &  6666 for damage</div>
                ...
                <div class="col-lg-12">                           
                  <div class="card ">
                    <div class="card-header">
                      <h3 class="h4 mb-0"> <i class="bi bi-card-checklist"></i> Approval</h3>
                    </div>
                    <div class="card-body ">
                      <div id="my-approval-recs" >
...
__mysys_apps.mepreloader('mepreloaderme',false);
    ...
    $('#btn-upload-wshe-gi').click(function(){ 
      try {   
        ...
        var file            = jQuery('#rcv-upld-file').val();
        var giType          = jQuery('#gi-type').val();
        var giRemarks       = jQuery('#gi-remarks').val();
        ...
        if($.trim(giType) == ''){ 
          ...
          jQuery('#memsgtestent_danger_bod').html('Select Type!');
          jQuery('#memsgtestent_danger').modal('show');
          return false;
        }
        ...
        if($.trim(file) == ''){ 
          ...
          jQuery('#memsgtestent_danger_bod').html('Please select file to upload!');
          jQuery('#memsgtestent_danger').modal('show');
          return false;
        }
        ...
        my_data.append('giType', giType);
        my_data.append('giRemarks', giRemarks);

   
         __mysys_apps.mepreloader('mepreloaderme',true);
        $.ajax({ // default declaration of ajax parameters
          url: '<?=site_url()?>gi-entry-upld',
          method:"POST",
...
function myrcvngcd_view_recs(mtkn_whse){ 
  ...
  mywg_gi_wf();
  function mywg_gi_wf(mtkn_arttr) { 
      var ajaxRequest;

      ajaxRequest = jQuery.ajax({
          url: "<?=site_url();?>gi-approval-vw",
          type: "post",
          data: {
...
// Deal with the results of the above ajax call
      ajaxRequest.done(function(response, textStatus, jqXHR) {
          jQuery('#my-approval-recs').html(response);
// and do it again
//setTimeout(get_if_stats, 5000);
      });
  };
