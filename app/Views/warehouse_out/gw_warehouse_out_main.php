...
<main id="main">
  ...
  ?>
  <div class="pagetitle">
      <h1>Outgoing-Shipping Dcoument </h1>
      <nav>
          <ol class="breadcrumb">
              <li class="breadcrumb-item"><a href="index.html">Home</a></li>
              <li class="breadcrumb-item active">Outgoing-Shipping Dcoument Entry</li>
          </ol>
          </nav>
  </div><!-- End Page Title -->
...
<!--                            <div class="input-group input-group-sm mb-3 mt-3">
                        ...
                        <div class="col-lg-8 "> 
                             <div class="row rounded p-2">
                              ...
                              <div class="mt-2 col-lg-6 ">
                                  <label for="helper-two"></label>
                                  <div class="input-group">
                                    <div class="input-group-text">
                                    <input type="checkbox" class="green-cb" id="is-mkg" style="scale:1.3"  <?=$mkg_check?>> 
                                    </div>
                                    <input type="text" class=" form-control form-control-sm" readonly placeholder="Check if MKG">
                                  </div>
                              </div>
                        ...
                        <div id="out-list">
                          ...
                          if(!empty($mtkn_trxno)):
                              ...
                              echo view('warehouse_out/warehouse_out_item_scanned',$dtData);
                          endif;
                           ?>
                        </div>
  ...
  $('#btn-out-search').on('click',function(){
    try{
        ...
        $.ajax({ // default declaration of ajax parameters
              type: "POST",
              url: '<?=site_url()?>warehouse-out-show',
...
function whseout_ent_recs(mtkn_whse){ 
    var ajaxRequest;

    ajaxRequest = jQuery.ajax({
        url: "<?=site_url();?>warehouse-out-recs",
        type: "post",
        data: {
            mtkn_whse: mtkn_whse
        }
...
$('#anchor-list').on('click',function(){
    $('#anchor-list').addClass('active');
    $('#anchor-items').removeClass('active');
    whseout_ent_recs($('#txt-warehouse').attr("data-id"));
...
$('#btn-update-header').on('click',function(){
  ...
  $.ajax({ // default declaration of ajax parameters
    type: "POST",
    url: '<?=site_url()?>warehouse-out-hd-updt',
    context: document.body,
    data: eval(mparam),
    global: false,
    cache: false,
