...
$mytxtsearchrec_tredempt = $request->getVar('txtsearchedrec');
...
<?=form_open('me-tot-redempt-recs','class="needs-validation-search" id="myfrmsearchrec_tredempt" ');?>
    <div class="col-md-6 mb-1">
        <div class="input-group input-group-sm">
            <label class="input-group-text fw-bold" for="search">Search:</label>
            <input type="text" id="mytxtsearchrec_tredempt" class="form-control form-control-sm" name="mytxtsearchrec_tredempt" placeholder="Search" />
...
<div class="col-md-8">
	<?=$mylibzsys->mypagination($npage_curr,$npage_count,'__myredirected_rsearch_tredempt','');?>
	</div>

			<div class="table-responsive">
				<div class="col-md-12 col-md-12 col-md-12">
					<table class="table table-striped table-hover table-bordered table-sm" id="tbldata_totredempt">
						<thead>
							<tr>
						...
						<tbody>
							...
							if($rlist !== ''):
								...
								<tr bgcolor="<?=$bgcolor;?>" <?=$on_mouse;?>>
									<td nowrap><?=$row['trxno'];?></td>
									<td nowrap><?=$row['BRNCH_NAME'];?></td>
									<td nowrap><?=$row['redeem_points'];?></td>
								</tr>
								<?php 
								$nn++;
								endforeach;
...
<script type="text/javascript"> 
    ...
    function meSetCellPadding () {
        var metable = document.getElementById ("tbldata_totredempt");
	...
	function __myredirected_rsearch_tredempt(mobj) { 
		try { 
			__mysys_apps.mepreloader('mepreloaderme',true);
			var txtsearchedrec = jQuery('#mytxtsearchrec_tredempt').val();
			...
			type: "POST",
			url: '<?=site_url();?>me-tot-redempt-recs',
	...
	jQuery('#mytxtsearchrec_tredempt').keypress(function(event) { 
		if(event.which == 13) { 
			...
			try { 
				...
				cache: false,
					success: function(data)  { //display html using divID
							jQuery('#totredemptrecs').html(data);
							__mysys_apps.mepreloader('mepreloaderme',false);
							return false;
					},
					error: function() { // display global error on the menu function
