...
<main id="main">
  ...
  <div class="pagetitle">
      ...
      <nav>
          <ol class="breadcrumb">
              <li class="breadcrumb-item"><a href="<?php site_url() ?>warehouse-inv-transfer">Home</a></li>
              <li class="breadcrumb-item active">Warehouse Transfer</li>
              <li class="breadcrumb-item transfer-manual-anchor" ><a href="<?php site_url() ?>warehouse-inv-transfer#rackbintrans-div">Rack & bin Transfer</a></li>
              <li class="breadcrumb-item transfer-upld-anchor"><a href="<?php site_url() ?>warehouse-inv-transfer#rackbintrans-upload-div">Rack & bin Transfer Upload</a></li>
        </ol>
        </nav>
  </div><!-- End Page Title -->
...
__mysys_apps.mepreloader('mepreloaderme',false);
    ...
    function myinv_rackbintrans_upload(mtkn_whse){ 
      ...
      myrcvngcd_view_transrecs(jQuery('#txt-warehouse').attr("data-id"));
      function myrcvngcd_view_transrecs(mtkn_whse){ 

          setTimeout(() => {
            var ajaxRequest;
            ajaxRequest = jQuery.ajax({
                url: "<?=site_url();?>warehouse-inv-transfer-recs",
                type: "POST",
                data: {
                    mtkn_whse: mtkn_whse
                }
            ...
            __mysys_apps.mepreloader('mepreloaderme',true);
              ajaxRequest.done(function(response, textStatus, jqXHR) {
                  jQuery('#transfer-recs').html(response);
                  __mysys_apps.mepreloader('mepreloaderme',false);
                  //$.hideLoading();
                  // and do it again
                  //setTimeout(get_if_stats, 5000);
      ...
      jQuery('#txt-plant' ) 
        ...
        jQuery('#txt-warehouse' ) 
          ...
          select: function( event, ui ) {
              ...
              myinv_rackbintrans(wshe_id);
              myinv_rackbintrans_upload(wshe_id);
              myrcvngcd_view_transrecs(wshe_id);
              this.value = ui.item.value; 
