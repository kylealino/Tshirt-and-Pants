...
public function auto_add_lines_pullout() { 
	...
	public function gr_rcvng_show(){

		$result = $this->mydatazua->get_Active_menus($this->db_erp,$this->sysuaid,"myuatrx_id='206'","myua_trx");
			if($result == 1):
			 //echo "string";
			echo view('templates/myheader');
			echo view('good_receive_rcvng/warehouse_gr_rcvng_main');
			echo view('templates/myfooter');
			else:
	    	echo view('templates/myheader');
			echo view('errors/html/error_404',$this->data);
	...
	public function gr_ent_rcvng_recs(){
		...
		$data    = $this->mytrxgr->view_rcvng_recs();
		if($data['response']):
		return view('good_receive_rcvng/warehouse_gr_rcvng_recs',$data);
		else:
		$dta['msg'] = 'No records found!';
		return view('components/no-records',$dta);
		endif;
	...
	public function wshe_gr_rvng_upld(){
		...
		$data = $this->mytrxgr->wh_gr_rcvng_upld();

		if($data['response'] == true){
			return view('good_receive_rcvng/warehouse_gr_upload_recs',$data);
		}
		else{
			echo $data['result'];
		}
	...
	public function wshe_gr_rcvng_save()
	{
		...
		$this->mytrxgr->mywh_gr_rcvng_save();
	}

	public function wshe_gr_rcvng_itm_recs() 
	{ 
		...
		$data    = $this->mytrxgr->view_ent_itm_recs($mpages,50,$txtsearchedrec);
		return view('good_receive_rcvng/warehouse_gr_rcvng_item_recs',$data);
	}
