...
class MyPurchaseModel extends Model
{
    ...
    public function purch_entry_save() {
       ...
       //PO CLASS
        ...
        if(!empty($mtkn_mntr)) { 
           //CHECK IF VALID PO
            $str = "select aa.recid,aa.po_sysctrlno from {$this->db_erp}.gw_po_hd aa where sha2(concat(aa.recid,'{$mpw_tkn}'),384) = '$mtkn_mntr'";
            $q = $this->mylibzdb->myoa_sql_exec($str,'URI: ' . $_SERVER['PHP_SELF'] . chr(13) . chr(10) . 'File: ' . __FILE__  . chr(13) . chr(10) . 'Line Number: ' . __LINE__);
            if($q->resultID->num_rows == 0) {
        ...
        //INSERT
            ...
            $cseqn =  $this->mydataz->get_ctr_new_dr('PO','',$this->db_erp,'CTRL_GWPO');//TRANSACTION NO
        } //end else
        ...
        if(count($adata1) > 0) { 
            ...
            if(count($adatar1) > 0) { 
                if(!empty($mtkn_mntr)) {       
                     $str = "
                        update {$this->db_erp}.`gw_po_hd` set 
                    ...
                    $str = "
                        insert into {$this->db_erp}.`gw_po_hd` (
                        `po_sysctrlno`,
                        `po_type_id`,
                        `po_cls_id`,
                        `po_stat_id`,
                      ...
                      //GET ID
                      $str = "select recid,sha2(concat(aa.recid,'{$mpw_tkn}'),384) mtkn_potr from {$this->db_erp}.`gw_po_hd` aa where `po_sysctrlno` = '$cseqn' ";
                      $q = $this->mylibzdb->myoa_sql_exec($str,'URI: ' . $_SERVER['PHP_SELF'] . chr(13) . chr(10) . 'File: ' . __FILE__  . chr(13) . chr(10) . 'Line Number: ' . __LINE__);  
                      $rr = $q->getRowArray();
                      $mpo_rid = $rr['recid'];
                    ...
                    for($xx = 0; $xx < count($adatar1); $xx++) { 
                       ...
                       // $gen_id = trim($xdata[22]);
                        ...
                        if(empty($mtkn_mntr)) {  
                            $str = "
                            insert into {$this->db_erp}.gw_po_dt ( 
                                `pohd_rid`,
                                `po_sysctrlno`,
                                `art_rid`,
                                `mat_code`,
                          ...
                          $this->mylibzdb->user_logs_activity_module($this->db_erp,'GW_PO_DT_ADD','',$cseqn,$str,'URI: ' . $_SERVER['PHP_SELF'] . chr(13) . chr(10) . 'File: ' . __FILE__  . chr(13) . chr(10) . 'Line Number: ' . __LINE__); 
                            ...
                            if(empty($podt_rid)) { 
                              ...
                              $this->mylibzdb->user_logs_activity_module($this->db_erp,'GW_PO_DT_ADD','',$cseqn,$str,'URI: ' . $_SERVER['PHP_SELF'] . chr(13) . chr(10) . 'File: ' . __FILE__  . chr(13) . chr(10) . 'Line Number: ' . __LINE__); 
                                ...
                                $str = "
                                select recid from {$this->db_erp}.`gw_po_dt` aa where sha2(concat(aa.recid,'{$mpw_tkn}'),384) = '$podt_rid'
                                ";
                              $qq = $this->mylibzdb->myoa_sql_exec($str,'URI: ' . $_SERVER['PHP_SELF'] . chr(13) . chr(10) . 'File: ' . __FILE__  . chr(13) . chr(10) . 'Line Number: ' . __LINE__);  
                                if($qq->resultID->num_rows > 0) {
                                    ...
                                    $str = "
                                    update {$this->db_erp}.`gw_po_dt` set 
                                    `art_rid` = '$mat_rid',
                                    `mat_code` = '$cmat_code',
                                    `convf` = '$nconvf',
                                    `qty` = '$nqty',
    ...
    public function po_barcde_gnrtion() {
        ...
        if(!empty($po_code)) { 
                $str2 = "
                select `is_bcodegen`,`is_approved`,`po_sysctrlno` FROM {$this->db_erp}.`gw_po_hd`
                ...
                if($PO_CFRM_TAG == 1){ 
                       ...
                       $str = "
                        ...
                        $str = "
                            INSERT INTO
                            {$this->db_erp}.`gw_wshe_barcdng_hd`(
                                `trx`,
                                `header`,
                                `muser`,
                                `encd`
                        ...
                        foreach($rrec as $rr){
                            ...
                            foreach($rrec as $row){
                                ...
                                for($i = 0; $i < $no_of_box; $i++){
                                    ...
                                    $str = "
                                        INSERT INTO
                                        {$this->db_erp}.`gw_wshe_barcdng_dt`(
                                            `trx`,
                                            `header_id`,
                                            `header`,
                                            `stock_code`,
                                    ...
                                    $itemq = $this->mylibzdb->myoa_sql_exec($str,'URI: ' . $_SERVER['PHP_SELF'] . chr(13) . chr(10) . 'File: ' . __FILE__  . chr(13) . chr(10) . 'Line Number: ' . __LINE__);
                                     if($itemq->resultID->num_rows > 0) { 
                                        ...
                                        foreach($rrec as $row){
                                           ...
                                           $str = "
                                                INSERT INTO {$this->db_erp}.`gw_wshe_barcdng_item`(
                                                    `header`,
                                                    `header_id`,
                                                    `dt_id`,
                                                    `mat_rid`,
    ...
    public function po_for_approval() {
        ...
        if(!empty($mtkn_potr)) { 
            ...
            $str = "select is_approved,po_sysctrlno from {$this->db_erp}.`gw_po_hd` aa WHERE sha2(concat(`recid`,'{$mpw_tkn}'),384) = '$mtkn_potr' AND `is_approved` = '2'";
            $qry = $this->mylibzdb->myoa_sql_exec($str,'URI: ' . $_SERVER['PHP_SELF'] . chr(13) . chr(10) . 'File: ' . __FILE__  . chr(13) . chr(10) . 'Line Number: ' . __LINE__);
